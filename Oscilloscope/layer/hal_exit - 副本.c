...
/*----------------------------------------------------------------------------*/
FOS_TSK_REGISTER(exit_thread,PRIORITY_0,1); /* gui detecter task run as 100 ms*/
/* static rot_flag */
	...
	if( rot_flag[index] == 0 )
	{
		...
		if( up_dm == 0 )
		{
			if( index == 2 )
			{
				...
				if( now - last_td > 5 * 1000 )
				{
					if(  now - last_td < 20*1000 )
					{
						rot_updm[index] += 20;
					}
					else if(  now - last_td < 40*1000 )
					{
						rot_updm[index] += 10;
					}
					else if(  now - last_td < 100*1000 )
					{
						rot_updm[index] += 5;
					}				
					else if(  now - last_td < 200*1000 )
					{
						rot_updm[index] += 3;
					}				
					else
